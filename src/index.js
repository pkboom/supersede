...
const MODES = ["address", "buttons", "both"];
const RULES = {
  addresses: [
    {
...
function replaceAddresses(source) {
  let html = source;
  let edits = 0;
...
function hasConfiguredButtonClass(attrs) {
...
function replaceButtonColors(source) {
  let edits = 0;
  ...
  const html = source.replace(tagRegex, (rawTag) => {
     ...
     */
    const bgcolorRegex = new RegExp(`(bgcolor\\s*=\\s*["']?)${escapeRegex(RULES.oldColor)}${COLOR_END}`, "gi");
    updatedTag = updatedTag.replace(bgcolorRegex, (_match, prefix) => {
      localEdits += 1;
      return `${prefix}${RULES.newColor}`;
    });
    /*
     ...
     */
    updatedTag = updatedTag.replace(/style\s*=\s*(["'])(.*?)\1/i, (_fullMatch, quote, styleValue) => {
...
function transform(source, mode = "both") {
  let html = source;
  const report = {
    addressEdits: 0,
    colorEdits: 0,
...
function copyFileCreatingDirectories(source, destination) {
  fs.mkdirSync(path.dirname(destination), {
    recursive: true,
  });
  fs.copyFileSync(source, destination);
}
function writeFileCreatingDirectories(destination, content) {
  fs.mkdirSync(path.dirname(destination), {
    recursive: true,
  });
...
Examples:
  ...
  node src/index.js ./original ./updated --mode address
  node src/index.js ./original ./updated --mode buttons
  node src/index.js ./original ./updated --mode both --apply
