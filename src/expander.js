 ...
 */
function findSurvivors(src, comments) {
  const out = [];
 ...
 */
function siblingIndexOf(source, offset, comments) {
...
function setAttrs(src, tag, updates) {
  ...
  return (
    src.slice(0, tag.start) + renderOpenTag(tag.name, attrs, tag.selfClosing) + src.slice(tag.end)
  );
}
/**
 ...
 */
function applyOverrides(body, overrides, componentId) {
 ...
 */
function stripSlotMarkers(body) {
 ...
 */
function replaceSlotText(body, slotName, value) {
...
export function expand(source, store, opts = {}) {
  const regions = [];
  ...
  if (survivors.length > 0) {
    const detail = survivors.map((at) => `offset ${at}: ${out.slice(at, at + 60)}`).join(" | ");
...
function expandInto(source, store, pins, regions, ancestry, depth, comments) {
  ...
  for (;;) {
    ...
    if (!componentId) {
      throw new ExpansionError(`<${COMPONENT_TAG}/> at offset ${tag.start} has no component-id`);
    }
    const pinned = pins[componentId];
    const revisionAttr = tag.attrs.find((a) => a.name === "revision")?.value;
    const revision = pinned ?? (revisionAttr === undefined ? NaN : Number(revisionAttr));
    ...
    const expandedBody = expandInto(body, store, pins, regions, chain, depth + 1, bodyScan.ranges);
