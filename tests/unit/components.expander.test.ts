...
const BUTTON_V1 =
  `<a href="https://shoe.test/shop" background-color="#1f6feb" color="#ffffff">Shop now</a>`;
const BUTTON_V2 =
  `<a href="https://shoe.test/shop" background-color="#111111" color="#ffffff">Shop now</a>`;
...
describe("store — immutable revisions", () => {
  ...
  it("rejects a self-referencing component", () => {
    ...
    expect(() =>
      s.publish(
        "a/b",
        `<tr><x-component component-id="a/b" revision="1" /></tr>`
      )
...
describe("expand — the throw-on-survivor guard", () => {
  ...
  it("throws for a reference with no revision", () => {
    expect(() =>
      expand(template(`<x-component component-id="shoe-brand/primary-button" />`), storeWithButton())
...
describe("expand — overrides (ov-*)", () => {
  ...
  it("overrides a named text slot", () => {
    ...
    s.publish(
      "brand/hero",
      `<tr><td><p data-slot="headline">Default</p></td></tr>`
    );
  ...
  it("strips the data-slot marker even when the slot is NOT overridden", () => {
    ...
    const { html } = expand(
      template(`<x-component component-id="brand/hero" revision="1" />`),
      s
    );
  ...
  it("removes ONLY the marker, leaving surrounding attribute formatting verbatim", () => {
    ...
    const { html } = expand(
      template(`<x-component component-id="brand/odd" revision="1" />`),
      s
    );
    expect(html).not.toContain("data-slot");
  ...
  it("does not touch a data-slot substring occurring inside another attribute value", () => {
    ...
    s.publish(
      "brand/txt",
      `<tr><td><p alt="data-slot=notreal">x</p></td></tr>`
    );
    const { html } = expand(
      template(`<x-component component-id="brand/txt" revision="1" />`),
      s
    );
  ...
  it("throws when a slot override targets a slot that does not exist", () => {
    ...
    expect(() =>
      expand(template(`<x-component component-id="brand/hero" revision="1" ov-slot-nope="y" />`), s)
...
describe("expand — nesting and the depth cap", () => {
  ...
  it("throws past the depth cap instead of recursing forever", () => {
    ...
    expect(() =>
      expand(template(`<x-component component-id="x" revision="1" />`), cyclic)
...
describe("expand — pins (the dry-run diff)", () => {
  it("pins override the revision written in the template", () => {
    ...
    expect(
      expand(src, s, { pins: { "shoe-brand/primary-button": 2 } }).html
    ).toContain("#111111");
  });
...
describe("comment scanning agrees with htmlparser2 (html's parser)", () => {
  ...
  it("expands a 200-reference document quickly enough for the render path", () => {
    ...
    const pad = Array.from({ length: 300 }, (_, i) => `<p>Filler ${i} with a realistic amount of body copy.</p>`).join("");
    const refs = Array.from({ length: 200 }, () => `<x-component component-id="b/btn" revision="1" />`).join("");
...
describe("unterminated comments in component BODIES (the fourth silent-200 path)", () => {
  ...
  it("publish() rejects an unterminated comment in a body", () => {
    const s = new InMemoryComponentStore();
    expect(() => s.publish("c/bad", `<span><!-- oops</span>`)).toThrow(
      UnterminatedCommentError
    );
  });
...
describe("below-root overrides (ov-at-<path>-<attr>)", () => {
  ...
  it("applies several overrides to different depths at once", () => {
    const { html } = expand(
      tpl(` ov-tag-0="img" ov-at-0-alt="Photo" ov-tag-2="a" ov-at-2-href="https://x.test" ov-padding="4px"`),
      card()
    );
  ...
  it("resolves a multi-segment path", () => {
    ...
    s.publish(
      "brand/wrap",
      `<tr><td><a href="https://default.test">Go</a></td></tr>`
    );
  ...
  it("without the guard the same rearrangement would land on the wrong node", () => {
    ...
    const { html } = expand(
      tpl(` ov-tag-2="p" ov-at-2-href="https://custom.test"`),
      after
    );
  ...
  it("rejects a malformed assertion path", () => {
    expect(() => expand(tpl(` ov-tag-abc="a"`), card())).toThrow(
  ...
  it("does not treat ov-tag-* as a root attribute", () => {
    const { html } = expand(
      tpl(` ov-tag-2="a" ov-at-2-href="https://x.test"`),
      card()
    );
